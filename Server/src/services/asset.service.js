...
const create = async ({ serial_number, tag, status, date_acq, description, model_id, location_id }) => {
  const [result] = await db.query(
    'INSERT INTO Asset (serial_number, tag, status, date_acq, description, model_id, location_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [serial_number, tag, status || 'Available', date_acq || null, description || null, model_id, location_id || null]
  );
  return findById(result.insertId);
};

const update = async (id, { serial_number, tag, status, date_acq, description, model_id, location_id }) => {
  await db.query(
    'UPDATE Asset SET serial_number = ?, tag = ?, status = ?, date_acq = ?, description = ?, model_id = ?, location_id = ? WHERE id = ?',
    [serial_number, tag, status, date_acq || null, description || null, model_id, location_id || null, id]
  );
  return findById(id);
};
